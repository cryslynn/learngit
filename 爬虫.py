...
def get_areas(url):
    ...
    # 模拟浏览器发送request请求,如不改写,resposne.request.headers = {'User-Agent': 'python-requests/2.18.4', 'Accept-Encoding': 'gzip, deflate', 'Accept': '*/*', 'Connection': 'keep-alive'}
    ...
    for i in range(1,len(areas)):  # areas[0]为不限,不想要不限
        area = areas[i]
...
def get_pages(area,area_link):
    ...
    for page in range(1,pages):
        url = area_link + 'pg' + str(page)   #之前有错误哦
...
def get_house_info(area, url):
    ...
    try:
        ...
        for i in range(30):  # 每一页有30个房源信息
            ...
            position = content.xpath("//div[@class='where']/span[3]/text()")[i].replace(" ", "")   # replace?
            try:
